...
##Set "install security updates"
		sudo cat /etc/apt/sources.list | grep "deb http://security.ubuntu.com/ubuntu/ trusty-security universe main multiverse restricted"
		if [ $? -eq 1 ]
		then
...
echo "$LogTime uss: [$UserName]# Deleting media files..." >> output.log
	...
	echo "###MEDIA FILES###" >> pFiles.log
    	sudo find / -name "*.mov" -type f >> pFiles.log
    	sudo find / -name "*.mp4" -type f >> pFiles.log
	sudo find / -name "*.mp3" -type f >> pFiles.log
	sudo find / -name "*.wav" -type f >> pFiles.log
	##Pictures
	echo "###PICTURES###" >> pFiles.log
#	find / -name "*.png" -type f >> pFiles.log
    sudo find / -name "*.jpg" -type f >> pFiles.log
	sudo find / -name "*.jpeg" -type f >> pFiles.log
	sudo find / -name "*.gif" -type f >> pFiles.log
	##Other Files
	echo "###OTHER###" >> pFiles.log
	sudo find / -name "*.tar.gz" -type f >> pFiles.log
	sudo find / -name "*.php" -type f >> pFiles.log
	sudo find / -name "*backdoor*.*" -type f >> pFiles.log
	sudo find / -name "*backdoor*.php" -type f >> pFiles.log
	##Items without groups
	echo "###FILES WITHOUT GROUPS###" >> pFiles.log
	sudo find / -nogroup >> pFiles.log
	sudo echo "###GAMES###" >> pFiles.log
	dpkg -l | grep -i game
...
echo "$LogTime uss: [$UserName]# Checking for firewall..." >> output.log
	case "$opsys" in
	"Ubuntu"|"Debian")
		sudo dpkg -l | grep ufw >> output.log
		if [ $? -eq 1 ]
		then
			sudo apt-get install ufw >> output.log
			sudo ufw deny 1337
			sudo ufw deny 23
			sudo ufw deny 2049
			sudo ufw deny 515
...
loginConf() {
	...
	"Debian") 
		...
		sudo sed -i 's/greeter-allow-guest=.*/greeter-allow-guest=false/' /etc/lightdm/lightdm.conf
		sudo sed -i 's/greeter-show-manual-login=.*/greeter-show-manual-login=true/' /etc/lightdm/lightdm.conf
		...
		sudo sed -i 's/autologin-user=.*/autologin-user=NONE/' /etc/lightdm/lightdm.conf

		sudo sed -i 's/^# disable-user-.*/disable-user-list=true/' /etc/gdm3/greeter.dconf-defaults
		sudo sed -i 's/^# disable-restart-.*/disable-restart-buttons=true/' /etc/gdm3/greeter.dconf-defaults
...
echo "$LogTime uss: [$UserName]# Creating /etc/lightdm/lightdm.conf for 12.04 compatability." >> output.log
		...
		then
			...
			sudo sed -i '$a greeter-show-manual-login=true' /etc/lightdm/lightdm.conf

			##Finds automatic login user if there is one and takes it out
			sudo cat /etc/ligthdm/lightdm.conf | grep autologin-user >> /dev/null
			if [ $? -eq 0 ]
			then
				...
				sudo sed -i '$a autologin-user=none' /etc/lightdm/lightdm.conf
			fi
...
echo "$LogTime uss: [$UserName]# Setting lockout policy..." >> output.log
	sudo sed -i 's/auth\trequisite\t\t\tpam_deny.so\+/auth\trequired\t\t\tpam_deny.so/' /etc/pam.d/common-auth
	sudo sed -i '$a auth\trequired\t\t\tpam_tally2.so deny=5 unlock_time=1800 onerr=fail' /etc/pam.d/common-auth
	sudo sed -i 's/sha512\+/sha512 remember=13/' /etc/pam.d/common-password
...
echo "$LogTime uss: [$UserName]# Checking for ssh..." >> output.log
	sudo dpkg -l | grep openssh-server >> output.log
        	if [ $? -eq 0 ];
        	then
                	read -p "Do you want SSH installed on the system?[y/n]: " a
                	if [ $a = n ];
                	then
                        	sudo apt-get autoremove -y --purge openssh-server ssh >> output.log
...
echo "$LogTime uss: [$UserName]# SSH has been found, securing now..." >> output.log
							...
							sudo sed -i '$a AllowUsers' /etc/ssh/sshd_config
							for x in `cat users`
							do
								sudo sed -i "/^AllowUser/ s/$/ $x /" /etc/ssh/sshd_config
							done
...
echo "$LogTime uss: [$UserName]# Installing and securing SSH now..." >> output.log
                        	sudo apt-get install -y openssh-server ssh >> output.log
				wait
				sudo ufw allow ssh
				sudo sed -i 's/LoginGraceTime .*/LoginGraceTime 60/g' /etc/ssh/sshd_config
                        	sudo sed -i 's/PermitRootLogin .*/PermitRootLogin no/g' /etc/ssh/sshd_config
...
function purgeUnauthorizedPackages() {
    ...
    badDefaults=(aisleriot cups "cups-*" "*-cups" gnome-mahjongg gnome-mines gnome-sudoku remmina "remmina*" samba sambashare "samba-*" tcpdump telnet tcpd netcat nc netcat "netcat*" smbd snmpd avahi "avahi-*" slapd ldap "ldap-*" nfs nfs-common nfs-kernel-server rsync talk irc nfs-server "nfs-*" rpcbind bind9 dovecot smbd squid snmpd inetutiles-inetd)
	...
	then
        	...
        	then
				...
				then
					...
					sudo chmod 640 /etc/php5/apache2/php.ini
				fi
        	fi
	else
...
##Looks for john the ripper
	sudo dpkg -l | grep john >> output.log
	if [ $? -eq 0 ];
	then
        	echo "JOHN HAS BEEEN FOUND! DIE DIE DIE"
        	sudo apt-get autoremove -y --purge john* >> output.log
...
##Look for HYDRA
	sudo dpkg -l | grep hydra >> output.log
	if [ $? -eq 0 ];
	then
		echo "HEIL HYDRA"
		sudo apt-get autoremove -y --purge hydra* >> output.log
		echo "HYDRA SLAIN"
	else
		echo "Hydra has not been found."
	fi
##Looks for nginx web server
	sudo dpkg -l | grep nginx >> output.log
	if [ $? -eq 0 ];
	then
...
echo "$LogTime uss: [$UserName]# Samba has been removed." >> output.log
			...
			sudo sed -i '82 i\restrict anonymous = 2' /etc/samba/smb.conf
			##List shares
		fi
	else
...
##LOOK FOR DNS
	...
	then
		...
		if [ $a = y ];
		then
			sudo apt-get autoremove -y --purge bind9
		fi
	else
		echo "DNS not found."
		sleep 1
...
##Looks for FTP
	sudo dpkg -l | grep ftp >> output.log
	if [ $? -eq 0 ]
	then	
		...
		then
			PID = `pgrep vsftpd`
			sudo sed -i 's/^/#/' /etc/vsftpd.conf
			sudo kill $PID
			sudo apt-get autoremove -y --purge vsftpd ftp
		else
...
##Looks for TFTPD
	sudo dpkg -l | grep tftpd >> output.log
	if [ $? -eq 0 ]
	then
		read -p "TFTPD has been installed, would you like to remove it?[y/n]: " a
		if [ $a = y ]
		then
			sudo apt-get autoremove -y --purge tftpd*
		fi
	else
...
##Looking for VNC
	sudo dpkg -l | grep vnc >> output.log
	if [ $? -eq 0 ]
	then
		read -p "VNC has been installed, would you like to remove it?[y/n]: " a
		if [ $a = y ]
		then
			sudo apt-get autoremove -y --purge vnc* 
		##else
			##Configure VNC
		fi
	else
...
##Looking for NFS
	sudo dpkg -l | grep nfs >> output.log
	if [ $? -eq 0 ]
	then	
		read -p "NFS has been found, would you like to remove it?[y/n]: " a
		if [ $a = y ]
		then
			sudo apt-get autoremove -y --purge nfs*
		##else
			##Configure NFS
		fi
	else
...
##Looks for snmp
	sudo dpkg -l | grep snmp >> output.log
	if [ $? -eq 0 ]
	then	
		echo "SNMP HAS BEEN LOCATED!"
		sudo apt-get autoremove -y --purge snmp*
	else
		echo "SNMP has not been found."
		sleep 1
	fi
##Looks for sendmail and postfix
	sudo dpkg -l | grep -E 'postfix|sendmail' >> output.log
	if [ $? -eq 0 ]
	then
		echo "Mail servers have been found."
		sudo apt-get autoremove -y --purge postfix sendmail
	else
		echo "Mail servers have not been located."
		sleep 1
	fi
##Looks xinetd
	sudo dpkg -l | grep xinetd >> output.log
	if [ $? -eq 0 ]
	then
		echo "XINIT HAS BEEN FOUND!"
		sudo apt-get autoremove -y --purge xinetd
	else
...
#then
	cat runningProcesses.log
		read -p "What is the name of the suspected netcat?[none]: " nc
			...
			sudo ls /etc/init | grep $nc.conf >> /dev/null
			if [ $? -eq 0 ]
			then
					sudo cat /etc/init/$nc.conf | grep -E -i 'nc|netcat|$ALIAS' >> /dev/null
					if [ $? -eq 0 ]
					then
							sudo sed -i 's/^/#/' /etc/init/$nc.conf
							sudo kill $PID
					else
							echo "This is not a netcat process."
					fi
			...
			sudo ls /etc/init.d | grep $nc >>/dev/null
			if [ $? -eq 0 ]
			then
					sudo cat /etc/init.d/$nc | grep -E -i 'nc|netcat|$ALIAS' >> /dev/null
					if [ $? -eq 0 ]
					then
							sudo sed -i 's/^/#/' /etc/init.d/$nc
							sudo kill $PID
					else
							echo "This is not a netcat process."
					fi
			...
			sudo ls /etc/cron.d | grep $nc >>/dev/null
			if [ $? -eq 0 ]
			then
					sudo cat /etc/cron.d/$nc | grep -E -i 'nc|netcat|$ALIAS' >> /dev/null
					if [ $? -eq 0 ]
					then
							sudo sed -i 's/^/#/' /etc/init.d/$nc
							sudo kill $PID
			...
			sudo ls /etc/cron.hourly | grep $nc >>/dev/null
			if [ $? -eq 0 ]
			then
					sudo cat /etc/cron.hourly/$nc | grep -E -i 'nc|netcat|$ALIAS' >> /dev/null
					if [ $? -eq 0 ]
					then
							sudo sed -i 's/^/#/' /etc/init.d/$nc
							sudo kill $PID
			...
			do
				sudo cat $x | grep '$nc|nc|netcat|$ALIAS'
				if [ $? -eq 0 ]
				then
					sudo sed -i 's/^/#/' /var/spool/cron/crontabs/$x
					sudo kill $PID
				else
			...
			sudo cat /etc/crontab | grep -i 'nc|netcat|$ALIAS'
			if [ $? -eq 0 ]
			then
			...
			sudo apt-get autoremove --purge netcat netcat-openbsd netcat-traditional
#else
...
sudoers() {
	...
	sudo cat /etc/sudoers | grep NOPASSWD.* >> /dev/null
	if [ $? -eq 0 ]
	then
	...
	sudo cat /etc/sudoers | grep timestamp_timeout >> /dev/null
	if [ $? -eq 0 ]
	then
...
##Creates any missing users
addGroup() {
	read -p "Would you like to add a user to a group?[y/n]: " a
	while [ $a = y ]
	do
		read -p "Please enter the name of the user: " user
		read -p "Please enter the name of the group: " group
		sudo usermod -a -G $group $user
		read -p "Are there any more users you would like to add to a group?[y/n]: " a
	done
